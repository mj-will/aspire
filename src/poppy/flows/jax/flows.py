...
class FlowJax(Flow):
    ...
    def __init__(self, dims: int, key=None, data_transform=None, **kwargs):
        ...
        if device is not None:
            logger.warning(
                "The device argument is not used in FlowJax. "
            )
    ...
    def forward(self, x, xp: Callable = jnp):
        ...
        return xp.asarray(z), xp.asarray(log_abs_det_jacobian + log_abs_det_jacobian_flow)
    ...
    def inverse(self, z, xp: Callable = jnp):
        ...
        return xp.asarray(x), xp.asarray(log_abs_det_jacobian + log_abs_det_jacobian_flow)
